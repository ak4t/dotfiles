...
Registry.registerRaw("emulation.js",function(){if(!window.console&&!console){console={log:function(c){tmCE.log((_background?"BG: ":"")+c)
...
b=null},xmlHttpRequest:function(c,l,j,m,i){var h=function(d){if(EMV){console.log("emu: req ("+e+" -"+c.url+" returned: "+d)
}if(d.onLoad&&l){if(d.r_id){d.response.response=window[d.r_id];
delete window[d.r_id]}l(d.response)}if(d.onReadyStateChange&&j){j(d.response)
}if(d.onError&&m){m(d.response)}if(d.onDone&&i){i(d.response)
}};var e=b.getResponseId(h);var k;if(typeof c.data==="object"){var g="__y__"+Math.floor(Math.random()*6121983+1);
window[g]=c;k=JSON.stringify({q_id:g})}else{k=JSON.stringify(c)
}var f=function(){tmCE.xmlHttpRequest(b.key,k,e)};window.setTimeout(f,1)
},runUpdateListeners:function(f,g,h){if(EMV){console.log("emu: runUpdateListeners "+f+" URL: "+h.url)
...
}tmCE.createTab(b.key,JSON.stringify(c))},getSelected:function(c,e){var d=b.getResponseId(e);
tmCE.getSelected(b.key,JSON.stringify(c),d)},update:function(c,d){tmCE.updateTab(b.key,c,d)
