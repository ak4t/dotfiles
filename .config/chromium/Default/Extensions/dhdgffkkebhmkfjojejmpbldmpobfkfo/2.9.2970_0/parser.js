...
if(j){k=j.join("")}else{k=f.Base64.encode(i).match(/[a-zA-Z0-9]/g).join("")
...
this.lastUpdated=0;this.sync={imported:false},this.options={comment:null,compatopts_for_requires:true,compat_metadata:false,compat_foreach:false,compat_arrayleft:false,compat_prototypes:false,compat_uW_gmonkey:false,compat_forvarin:false,noframes:false,awareOfChrome:false,run_at:"",user_agent:"",override:{includes:false,merge_includes:true,use_includes:[],orig_includes:[],matches:false,merge_matches:true,use_matches:[],orig_matches:[],excludes:false,merge_excludes:true,use_excludes:[],orig_excludes:[]}}
...
if(v.trim()!=""){w.downloadURL=v}}}if(w.name){w.id=d(w.name);
if(D){console.log("parser: script "+w.name+" got id "+w.id)
...
if((l>0&&l<n)||(k>0&&k<n)){return null}return o},createScriptFromSrc:function(j){j=j.replace(/\r/g,"\n");
j=j.replace(/\n\n/g,"\n");j=j.replace(/\r/g,"");var k=g.getHeader(j);
if(!k){return{}}var i=g.processHeader(k);i.textContent=j;
i.header=k;if(!i.options.awareOfChrome){i.options.compat_metadata=(j!=b.unMetaDataify(j));
