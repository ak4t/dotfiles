...
};var g=function(s,p){var l=s.message;if(p.length==1&&d.toType(p[0]==="Array")){p=p[0]
}for(var n in s.placeholders){try{var t=Number(s.placeholders[n].content.replace(/^\$/,""))-1;
var r;if(t<p.length){r=p[t];l=l.replace("$"+n+"$",r)
}else{console.log("i18n: invalid argument count on processing '"+l+"' with args "+JSON.stringify(p))
}}catch(q){console.log("i18n: error processing '"+l+"' with args "+JSON.stringify(p))
